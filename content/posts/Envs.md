...
- **流程规范：** 任何需求在经过如下四个阶段逐步讨论后开始实现，
    1. 首先与用户充分讨论并对齐需求，并将相关内容写入 spec.md；
    2. 根据 spec 规划实现可验证实现方案，充分讨论效率、兼容性、健壮性后写入 plan.md；
    3. 根据 plan 思考并设计验证测试，并在当前项目下予以实现；
    4. 输出 TODO 写入 task.md，完成一项更新一项。
- **交互规范：** 你应当尽可能提供提供结构化、可选择的表单和用户讨论，且表单中必须保留用户自由回复的选项。
